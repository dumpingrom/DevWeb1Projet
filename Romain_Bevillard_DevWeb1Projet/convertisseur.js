	...
	****************/
	$(document).on({
		click: function () {
			...
			if($("#converter").attr("data-calc-done") == "false") {
				/*
					CHIFFRES / POINT
				*/
				...
				*/
					...
					if (!isNaN(result)) {
						...
						$("#converter").attr("data-calc-done", "true");
					}
					else {
						input.css("background-color", red);
				...
				/*
					CHIFFRES / POINT
				*/
				//On reinitialise le champ et l'attribut data-calc-done
				if($(this).attr("id") != "equals") {
					input.val($(this).text());
					$("#converter").attr("data-calc-done", "false");
				}
			}
		}
	}, "#converter button");
